...
def are_cousins(self, i_id, f_id):
    try:
...
def add_to_waiting(self, i_id, f_id):
...
def rmv_from_waiting(self, i_id, f_id):
    in_waiting = SESSION.query(Wait).get(i_id, f_id)
    if in_waiting:
        SESSION.delete(in_waiting)
        SESSION.commit()  

def check_waiting_list(self, i_id, f_id):
