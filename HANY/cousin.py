...
async def csn(_, message: Message):
    ...
    if await is_waiting(f_id, i_id) is True:
    ...
    if f_id in WAITING_LIST:
        await add_cousin(i_id, f_id)
        await add_cousin(f_id, i_id)
        await rmv_from_waiting(f_id, i_id)
        ...
        await add_to_waiting(i_id, f_id)
